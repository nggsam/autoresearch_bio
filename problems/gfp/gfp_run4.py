"""
GFP Exp13: 2.6M model + 600s + single warm restart at 50%.
Hypothesis: Exp11 peaked at 67% then degraded. A restart at 50% gives two chances to
find good minima, and top-K ensemble combines both runs' best checkpoints.
Usage: modal run gfp_run4.py
"""

import modal, os, time

app = modal.App("gfp-direct-v4")
...
def train_gfp():
    ...
    while True:
        ...
        for xb, yb in loader:
            ...
            # Two-phase cosine LR with warm restart at RESTART_AT
            if prog < 0.03:
                lr_m = prog / 0.03
            elif prog < RESTART_AT:
                # Phase 1: cosine from 1.0 to 0.01
                cp = (prog - 0.03) / (RESTART_AT - 0.03)
                lr_m = max(0.01, 0.5*(1+math.cos(math.pi*cp)))
            elif prog < RESTART_AT + 0.03:
                # Restart warmup
                rp = (prog - RESTART_AT) / 0.03
                lr_m = rp
                if not restarted:
                    restarted = True
                    print(f"\n  [RESTART] at prog={prog:.2f}")
            else:
                # Phase 2: cosine from 1.0 to 0.01
...
def main():
    print("Launching GFP Exp13...")
    t0 = time.time()
    r = train_gfp.remote()
    print(f"\n{'='*60}\nEXP13 COMPLETE ({time.time()-t0:.0f}s)\n{'='*60}")
